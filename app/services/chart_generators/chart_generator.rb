  module ChartGenerators
    class ChartGenerator
      ...
      def setup_gauge_chart(chart)
        puts "Setting up gauge chart for #{@metric[:name]}"
        current_value = @metric[:data].first[:value].to_f
        target_value = 145  # Fixed target value
        max_value = [@metric[:data].map { |d| d[:value].to_f }.max, target_value].max * 1.2
        min_value = 0
        ...
        current_angle = start_angle + (current_value / max_value) * Math::PI
        target_angle = start_angle + (target_value / max_value) * Math::PI
        ...
        chart
          .layer([
            ...
            # Green arc (up to target)
            {
              ...
              encoding: {
                color: { value: '#4daf4a' }  # Green
              }
            },
            # Blue arc (over target)
            {
              mark: { 
                type: :arc, 
                innerRadius: 120, 
      ...
      def add_special_features(chart)
        ...
        when 'line', 'bar', 'area', 'point'
          ...
          puts "Unknown chart type: #{@presentation.chart_type} for #{@metric[:name]}"
          chart
        end
      end
